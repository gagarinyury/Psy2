...
from app.core.policies import gated_access_allowed, is_risk_trigger, effective_disclosure_level, DisclosureRules
...
def test_helpers():
    ...
    assert gated_access_allowed(0.5, 0.4) == True
    assert gated_access_allowed(0.3, 0.4) == False
    assert gated_access_allowed(0.4, 0.4) == True
    
    # is_risk_trigger
    assert is_risk_trigger("Бывают ли мысли о суициде?", ["суицид"]) == True
    assert is_risk_trigger("Как дела?", ["суицид"]) == False
    assert is_risk_trigger("СУИЦИД", ["суицид"]) == True  # case insensitive
