...
async def test_session_rate_limit_with_header(fake_redis):
    ...
    with (
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            response = await client.post(
                "/turn", json=payload, headers={"X-Session-ID": session_id_2}
            )
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            for i in range(200):
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Request {i}: Status {response.status_code}, Response: {response.text[:200]}"
                    )
                    if i == 0:  # Show first error
                        break

            assert success_count == 200, (
                f"All 200 requests should succeed when rate limiting disabled, got {success_count}"
            )
