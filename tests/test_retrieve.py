...
async def setup_test_data(session):
    ...
    with open(demo_case_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    case_id = await load_case(session, data['case'], data['kb'])
...
async def test_high_trust_threshold_gated_access(session):
    ...
    result_low = await retrieve(session, 
        case_id=case_id,
        intent="open_question",
        topics=topics,
    ...
    result_high = await retrieve(session, 
        case_id=case_id,
        intent="open_question",
        topics=topics,
...
@pytest.mark.asyncio
async def test_nonexistent_case_id(session):
    """
...
@pytest.mark.asyncio
async def test_empty_database_scenario(session):
    """
    Тест с пустой БД
    """
    # Очистка данных в БД
