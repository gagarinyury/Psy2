...
async def test_cli_case_loader():
    ...
    test_version = f"test-1.0-{uuid.uuid4().hex[:8]}"
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert len(case_fragments) == 2, f"Expected 2 KB fragments for case, got {len(case_fragments)}"
        ...
        public_count_result = await session.execute(
            text("SELECT COUNT(*) FROM kb_fragments WHERE case_id = :case_id AND metadata->>'availability' = 'public'"),
            {"case_id": case_id}
        )
        ...
        gated_count_result = await session.execute(
            text("SELECT COUNT(*) FROM kb_fragments WHERE case_id = :case_id AND metadata->>'availability' = 'gated'"),
            {"case_id": case_id}
        )
        ...
        assert len(public_fragments) == 1, f"Expected 1 public fragment via ORM, got {len(public_fragments)}"
        assert len(gated_fragments) == 1, f"Expected 1 gated fragment via ORM, got {len(gated_fragments)}"
        ...
        assert "Родился в 1989" in public_fragment.text, f"Expected birth year in text, got {public_fragment.text}"
        
        gated_fragment = gated_fragments[0]
        assert gated_fragment.type == "symptom", f"Expected symptom type, got {gated_fragment.type}"
        assert "Нарушение сна" in gated_fragment.text, f"Expected sleep disorder in text, got {gated_fragment.text}"
