...
@pytest.mark.asyncio
async def test_pipeline_turn_integration():
    ...
    async with AsyncClient(
        ...
        assert "Echo:" in turn_response["patient_reply"], "patient_reply should contain 'Echo:'"
        assert turn_response["risk_status"] == "none", f"Expected risk_status 'none', got {turn_response['risk_status']}"
        ...
        assert turn_response["risk_status"] == "acute", f"Expected risk_status 'acute', got {turn_response['risk_status']}"
        assert "eval_markers" in turn_response
        assert "intent" in turn_response["eval_markers"]
        assert turn_response["eval_markers"]["intent"] == "risk_check", f"Expected intent 'risk_check', got {turn_response['eval_markers']['intent']}"
...
@pytest.mark.asyncio
async def test_pipeline_turn_sleep_scenario():
    ...
    async with AsyncClient(
        ...
        # Detailed validation
        assert data["patient_reply"].startswith("Echo:"), "patient_reply should start with 'Echo:'"
...
@pytest.mark.asyncio
async def test_pipeline_turn_risk_scenario():
...
@pytest.mark.asyncio
async def test_pipeline_turn_response_structure():
    ...
    async with AsyncClient(
        ...
        required_fields = ["patient_reply", "state_updates", "used_fragments", "risk_status", "eval_markers"]
