...
async def test_session_skips_ip_even_if_ip_limit_zero(client, monkeypatch, app):
    ...
    assert resp.status_code != 429, f"Expected non-429 status but got {resp.status_code}: {resp.text}"
...
async def test_no_session_uses_ip_limit(client, monkeypatch, app):
    ...
    assert r2.status_code == 429, f"Second request should be rate limited, got: {r2.status_code} {r2.text}"
...
async def test_session_blocks_without_ip_check(client, monkeypatch, app):
    ...
    assert blk.status_code == 429, f"Second request should be blocked, got: {blk.status_code} {blk.text}"
