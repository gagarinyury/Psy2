...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_kb_fragments_consistency_keys",
        "kb_fragments",
        ["consistency_keys"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_sessions_session_state",
        "sessions",
        ["session_state"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_telemetry_used_fragments",
        "telemetry_turns",
        ["used_fragments"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_telemetry_eval_markers",
        "telemetry_turns",
        ["eval_markers"],
        postgresql_using="gin",
    )
    ...
    op.execute(
        "CREATE INDEX ix_kb_fragments_embedding_hnsw ON kb_fragments USING hnsw (embedding vector_cosine_ops)"
    )
